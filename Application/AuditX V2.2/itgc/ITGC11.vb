...
Module ITGC11
    ' Subroutine to check Ability to import transports to production environment
    ...
    Sub ExecuteITGC11()
        ...
        Try
            ...
            ' Ability to import transports to production environment
            '-----------------------------------------------------------------------------------------------------------------
            ...
            CopyUsersToClipboard(My.Settings.ITGC11_ApprovedUsers)
            ...
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL101").text = "STMS"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL102").text = "STMS_IMPORT"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ2").Text = "S_CTS_ADMI"
            ...
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL201").Text = "IMPA"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL203").Text = "IMPS"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL203").setFocus
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL203").caretPosition = 3
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ3").Text = "S_TRANSPRT"
            ...
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL311").Text = "03"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL311").SetFocus
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL311").caretPosition = 3
            session.findById("wnd[0]").sendVKey(0)
            Threading.Thread.Sleep(waitTime)
            ...
            Else
                LogMessage1 = "User found with Ability to import transports to production environment"
                Try
                ...
                Dim fileNameWithExt As String = $"{controlITGC} {systemname} User with Ability to import transports to production environment.xls"
            ...
            '======== Checkpoint 1.1 - Roles with Ability to import transports to production environment ========
            Try
                ...
                If LogMessageRoles1.Contains("No matching user found") Then
                    ...
                    AddDirectProfileNote(WordDoc, "Ability to import transports to production environment")
                    LogMessageRoles1 = "No role found - user(s) may have access to import transports via direct profile assignment"
                    Logger.LogMessage("Check 1.1: No role found - direct profile assignment suspected", False)
                    ' ----------------------------------------
                Else
                    LogMessageRoles1 = "Role found with Ability to import transports to production environment"
                    Try
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(1, 251)
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(2, -1)
                    Catch ex As Exception
                    ...
                    session.findById("wnd[1]/tbar[0]/btn[0]").press
                    session.FindById("wnd[1]/usr/ctxtDY_PATH").Text = folderPath
                    Dim fileNameWithExt As String = $"{controlITGC} {systemname} User's role with Ability to import transports to production environment.xls"
                    Dim exportPath As String = FileUtils.GetUniqueFileName(folderPath, fileNameWithExt)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").Text = Path.GetFileName(exportPath)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").caretPosition = 5
                    session.FindById("wnd[1]/tbar[0]/btn[0]").Press()
                ...
                If roleCount < userCount Then
                    LogMessageRoles1 &= " Some user have access to import transports to production environment via direct profile assignment"
                End If
