...
Module ITGC12
    ' Subroutine to check Ability to perform job administration functions
    ...
    Sub ExecuteITGC12()
        ...
        Try
            ...
            ' Ability to perform job administration functions
            '-----------------------------------------------------------------------------------------------------------------
            ...
            CopyUsersToClipboard(My.Settings.ITGC12_ApprovedUsers)
            ...
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL101").text = "SM37"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ2").text = "S_BTCH_ADM"
            ...
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL201").text = "Y"
            ...
            Else
                LogMessage1 = "User found with Ability to perform job administration functions"
                Try
                ...
                Dim fileNameWithExt As String = $"{controlITGC} {systemname} User with Ability to perform job administration functions.xls"
            ...
            '======== Checkpoint 1.1 - Roles with Ability to perform job administration functions ========
            Try
                ...
                If LogMessageRoles1.Contains("No matching user found") Then
                    ...
                    AddDirectProfileNote(WordDoc, "Ability to perform job administration functions")
                    LogMessageRoles1 = "No role found - user(s) may have access to perform job administration functions via direct profile assignment"
                    Logger.LogMessage("Check 1.1: No role found - direct profile assignment suspected", False)
                    ' ----------------------------------------
                Else
                    LogMessageRoles1 = "Role found with Ability to perform job administration functions"
                    Try
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(1, 251)
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(2, -1)
                    Catch ex As Exception
                    ...
                    session.findById("wnd[1]/tbar[0]/btn[0]").press
                    session.FindById("wnd[1]/usr/ctxtDY_PATH").Text = folderPath
                    Dim fileNameWithExt As String = $"{controlITGC} {systemname} User's role with Ability to perform job administration functions.xls"
                    Dim exportPath As String = FileUtils.GetUniqueFileName(folderPath, fileNameWithExt)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").Text = Path.GetFileName(exportPath)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").caretPosition = 5
                    session.FindById("wnd[1]/tbar[0]/btn[0]").Press()
                ...
                If roleCount < userCount Then
                    LogMessageRoles1 &= " Some user have access to perform job administration functions via direct profile assignment"
                End If
