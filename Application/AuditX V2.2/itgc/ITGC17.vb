...
Module ITGC17
    ' Subroutine to check Authorizations to Modify IDOCs in Production
    ...
    Sub ExecuteITGC17()
        ...
        Try
            ...
            ' Authorizations to Modify IDOCs in Production
            '-----------------------------------------------------------------------------------------------------------------
            ...
            CopyUsersToClipboard(My.Settings.ITGC17_ApprovedUsers)
            ...
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL101").text = "BD87"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL101").setFocus
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL101").caretPosition = 4
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL102").text = "WE19"
            ...
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ2").text = "S_IDOCCTRL"
            ...
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL203").text = "16"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL203").setFocus
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL203").caretPosition = 2
            session.findById("wnd[0]").sendVKey(0)
            ...
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL211").caretPosition = 1
            session.findById("wnd[0]").sendVKey(0)
            ...
            Else
                LogMessage1 = "User found with Ability to Modify IDOCs in Production"
                Try
                ...
                Dim fileNameWithExt As String = $"{controlITGC} {systemname} User with Ability to Modify IDOCs in Production.xls"
            ...
            '======== Checkpoint 1.1 - Roles with Ability to Modify IDOCs in Production ========
            ...
            Try
                ...
                If LogMessageRoles1.Contains("No matching user found") Then
                    ...
                    AddDirectProfileNote(WordDoc, "Ability to Modify IDOCs in Production")
                    LogMessageRoles1 = "No role found - user(s) may have access to Modify IDOCs in Production via direct profile assignment"
                    Logger.LogMessage("Check 1.1: No role found - direct profile assignment suspected", False)
                    ' ----------------------------------------
                Else
                    LogMessageRoles1 = "Role found with Ability to Modify IDOCs in Production"
                    Try
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(1, 251)
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(2, -1)
                    Catch ex As Exception
                    ...
                    session.findById("wnd[1]/tbar[0]/btn[0]").press
                    session.FindById("wnd[1]/usr/ctxtDY_PATH").Text = folderPath
                    Dim fileNameWithExt As String = $"{controlITGC} {systemname} User's role with Ability to Modify IDOCs in Production.xls"
                    Dim exportPath As String = FileUtils.GetUniqueFileName(folderPath, fileNameWithExt)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").Text = Path.GetFileName(exportPath)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").caretPosition = 5
                    session.FindById("wnd[1]/tbar[0]/btn[0]").Press()
                ...
                If roleCount < userCount Then
                    LogMessageRoles1 &= " Some user have access to Modify IDOCs in Production via direct profile assignment"
                End If
            ...
            Else
                ...
                SaveITGCComment(
                    ...
                    systemName:=systemname,
                    isGreen:=False,
                    comment:=$"[Check 1] - {LogMessage1} | {LogMessageRoles1}",
                    baseFolder:=ReprtOutputPath,
                    forMonth:=ReportMonth)
            End If
