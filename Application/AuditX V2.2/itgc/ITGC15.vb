...
Module ITGC15
    ' Subroutine to check Ability to change CUA configuration settings
    ...
    Sub ExecuteITGC15()
        ...
        Try
            ...
            ' Checkpoint 1 - Users with Access to maintain CUA landscape
            '-----------------------------------------------------------------------------------------------------------------
            ...
            ' ---- Clipboard method for ApprovedUsers - Checkpoint 1 ----
            CopyUsersToClipboard(My.Settings.ITGC15_ApprovedUsers)
            ...
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL101").text = "SCUA"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ2").text = "S_USER_SYS"
            ...
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL211").text = "78"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL211").setFocus
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL211").caretPosition = 2
            session.findById("wnd[0]").sendVKey(0)
            Threading.Thread.Sleep(waitTime)
            ...
            Else
                LogMessage1 = "User found with Access to maintain CUA landscape"
                Try
                ...
                Dim fileNameWithExt As String = $"{controlITGC} {systemname} User with Access to maintain CUA landscape.xls"
            ...
            '======== Checkpoint 1.1 - Roles with Access to maintain CUA landscape ========
            Try
                ...
                If LogMessageRoles1.Contains("No matching user found") Then
                    ...
                    AddDirectProfileNote(WordDoc, "Access to maintain CUA landscape")
                    LogMessageRoles1 = "No role found - user(s) may have access to maintain CUA landscape via direct profile assignment"
                    Logger.LogMessage("Check 1.1: No role found - direct profile assignment suspected", False)
                    ' ----------------------------------------
                Else
                    LogMessageRoles1 = "Role found with Access to maintain CUA landscape"
                    Try
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell/shellcont[1]/shell").SetRowSize(1, 251)
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell/shellcont[1]/shell").SetRowSize(2, -1)
                    Catch ex As Exception
                        Logger.LogMessage("Error setting row size: " & ex.Message, False)
                    End Try
                    ...
                    session.findById("wnd[1]/tbar[0]/btn[0]").press
                    session.FindById("wnd[1]/usr/ctxtDY_PATH").Text = folderPath
                    Dim fileNameWithExt As String = $"{controlITGC} {systemname} User's role with Access to maintain CUA landscape.xls"
                    Dim exportPath As String = FileUtils.GetUniqueFileName(folderPath, fileNameWithExt)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").Text = Path.GetFileName(exportPath)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").caretPosition = 5
                    session.FindById("wnd[1]/tbar[0]/btn[0]").Press()
                ...
                If roleCount < userCount Then
                    LogMessageRoles1 &= " Some user have Access to maintain CUA landscape via direct profile assignment"
                End If
            ...
            stepNum = 9
            ' Checkpoint 2 - Access to Maintain user distribution
            '-----------------------------------------------------------------------------------------------------------------
            session.findById("wnd[0]").maximize
            session.findById("wnd[0]/tbar[0]/okcd").text = "SUIM"
            Threading.Thread.Sleep(waitTime)
            ...
            ' ---- Clipboard method for ApprovedUsers - Checkpoint 2 ----
            CopyUsersToClipboard(My.Settings.ITGC15_ApprovedUsers)
            session.findById("wnd[1]/tbar[0]/btn[24]").press()     ' Upload from clipboard
            Threading.Thread.Sleep(500)
            session.findById("wnd[1]/tbar[0]/btn[8]").press()      ' Confirm
            ...
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ1").caretPosition = 7
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/txtVAL101").text = "SCUM"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ2").text = "S_USER_SYS"
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ2").setFocus
            session.findById("wnd[0]/usr/tabsTABSTRIP_TAB/tabpTAB4/ssub%_SUBSCREEN_TAB:RSUSR002:1004/ctxtOBJ2").caretPosition = 10
            session.findById("wnd[0]").sendVKey(0)
            ...
            Else
                LogMessage2 = "User found with Access to Maintain user distribution"
                Try
                    session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(1, 251)
                    session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SetRowSize(2, -1)
                Catch ex As Exception
                ...
                session.findById("wnd[1]/tbar[0]/btn[0]").press
                session.FindById("wnd[1]/usr/ctxtDY_PATH").Text = folderPath
                Dim fileNameWithExt As String = $"{controlITGC} {systemname} User with Access to Maintain user distribution.xls"
                Dim exportPath As String = FileUtils.GetUniqueFileName(folderPath, fileNameWithExt)
                session.FindById("wnd[1]/usr/ctxtDY_FILENAME").Text = Path.GetFileName(exportPath)
                session.FindById("wnd[1]/usr/ctxtDY_FILENAME").caretPosition = 5
                session.FindById("wnd[1]/tbar[0]/btn[0]").Press()
            ...
            '======== Checkpoint 2.1 - Roles with Access to Maintain user distribution ========
            Try
                session.findById("wnd[0]/usr/cntlGRID1/shellcont/shell/shellcont[1]/shell").currentCellRow = -1
                session.findById("wnd[0]/usr/cntlGRID1/shellcont/shell/shellcont[1]/shell").selectColumn("BNAME")
                session.findById("wnd[0]/usr/cntlGRID1/shellcont/shell/shellcont[1]/shell").selectedRows = ""
                ...
                If LogMessageRoles2.Contains("No matching user found") Then
                    ...
                    ' ---- Direct profile assignment note ----
                    AddDirectProfileNote(WordDoc, "Access to Maintain user distribution")
                    LogMessageRoles2 = "No role found - user(s) may have access to Maintain user distribution via direct profile assignment"
                    Logger.LogMessage("Check 2.1: No role found - direct profile assignment suspected", False)
                    ' ----------------------------------------
                Else
                    LogMessageRoles2 = "Role found with Access to Maintain user distribution"
                    Try
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell/shellcont[1]/shell").SetRowSize(1, 251)
                        session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell/shellcont[1]/shell").SetRowSize(2, -1)
                    Catch ex As Exception
                    ...
                    session.findById("wnd[1]/tbar[0]/btn[0]").press
                    session.FindById("wnd[1]/usr/ctxtDY_PATH").Text = folderPath
                    Dim fileNameWithExt As String = $"{controlITGC} {systemname} User's role with Access to Maintain user distribution.xls"
                    Dim exportPath As String = FileUtils.GetUniqueFileName(folderPath, fileNameWithExt)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").Text = Path.GetFileName(exportPath)
                    session.FindById("wnd[1]/usr/ctxtDY_FILENAME").caretPosition = 5
                    session.FindById("wnd[1]/tbar[0]/btn[0]").Press()
                ...
                If roleCount < userCount Then
                    LogMessageRoles2 &= " Some user have Access to Maintain user distribution via direct profile assignment"
                End If

            Catch ex As Exception
                Logger.LogMessage("Error during Checkpoint 2.1: " & ex.Message, False)
                Logger.LogException(ex, $"Error during Checkpoint 2.1 in {controlID}")
                stepNum = 400
                Threading.Thread.Sleep(waitTime)
                Takescreenshot(WordDoc, stepNum)                    ' Step 400
                ' ---- Direct profile assignment note on exception ----
            ...
            If (LogMessage1.Contains("No matching user found") And
                LogMessage2.Contains("No matching user found")) Then
                ' GREEN ZONE - Both checks passed
                SaveITGCComment(
            ...
            Else
                ' RED ZONE - One or both checks failed
                SaveITGCComment(
                    controlID:=controlITGC,
                    description:=controlName,
                    systemName:=systemname,
