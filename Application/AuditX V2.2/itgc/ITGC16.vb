...
Module ITGC16
    ...
    Sub ExecuteITGC16()
        ...
        Try
            ...
            session.findById("wnd[0]/usr/ctxtDATABROWSE-TABLENAME").text = "TRDIR"
            ...
            ' ---- Clipboard method for ProgramName ----
            ' Note: This popup uses tabpSIVA (Include tab) with txt (not ctxt) field
            ' so btn[24] pastes into the Include Values tab directly
            CopyValuesToClipboard(My.Settings.ITGC16_ProgramName)
            ...
            session.findById("wnd[0]/usr/ctxtI16-LOW").text = selectedStartDate.ToString("dd.MM.yyyy")
            session.findById("wnd[0]/usr/ctxtI16-LOW").setFocus
            session.findById("wnd[0]/usr/ctxtI16-LOW").caretPosition = 10
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/ctxtI16-HIGH").text = selectedEndDate.ToString("dd.MM.yyyy")
            session.findById("wnd[0]/usr/ctxtI16-HIGH").setFocus
            session.findById("wnd[0]/usr/ctxtI16-HIGH").caretPosition = 10
            session.findById("wnd[0]").sendVKey(0)
            Threading.Thread.Sleep(waitTime)
            ...
            Try
                LogMessage = session.FindById("wnd[0]/sbar").Text
            Catch ex As Exception
                Logger.LogMessage("Error retrieving log message: " & ex.Message, False)
                Logger.LogException(ex, "Error retrieving log message in ITGC16")
            End Try

            If LogMessage.Contains("No table entries found for specified key") Then
                LogMessage = "No SAP Standard Program changes retrived for the given date range."
                stepNum = 402
                Threading.Thread.Sleep(waitTime)
                Takescreenshot(WordDoc, stepNum)                    ' Step 402
            Else
                LogMessage = "SAP Standard Program changes retrived successfully for the given date range."
                ...
                Catch ex As Exception
                    ...
                    Logger.LogException(ex, $"Error setting row size in {controlID}")
                End Try
                ...
                Dim i1 As Integer = 0, j As Integer = visibleRows

                Do While ExecutionTime > 0
                    session.FindById("wnd[0]/usr/cntlGRID1/shellcont/shell").SelectedRows = i1
                    Threading.Thread.Sleep(waitTime)
                ...
                Dim fileNameWithExt As String = $"{controlITGC} {systemname} SAP Standard Program changes.xls"
            ...
            If LogMessage.Contains("No SAP Standard Program changes retrived for the given date range.") Then
                ' GREEN ZONE
                SaveITGCComment(
